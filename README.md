...
> A blackdot management framework built on **Feature Registry** (modular control plane), **Configuration Layers** (multi-machine settings), and **Claude Code Integration** (portable AI-assisted development). Designed for developers who work across machines. Everything is optional except shell config.
...
**These blackdot integrate with [dotclaude](https://github.com/blackwell-systems/dotclaude)** - a profile manager for Claude Code that syncs your AI assistant configurations across machines.
...
The `/workspace` symlink creates identical paths across platforms. Claude Code session folders match everywhere. Start on Mac, continue on Linux, full history intact. Multiple backends: Anthropic Max (consumer) or AWS Bedrock (enterprise SSO). No other blackdot does this.
...
Seamless integration with [dotclaude](https://github.com/blackwell-systems/dotclaude). dotclaude manages Claude profiles (CLAUDE.md, agents, standards). blackdot manages secrets (SSH, AWS, Git). Switch between OSS, client, and work contexts while vault secrets stay synced. Both respect `/workspace` paths for portable sessions (target directory is configurable via `WORKSPACE_TARGET`).
...
chezmoi is the most popular blackdot manager. Here's how we compare:
...
1. **Only blackdot built for Claude Code** - Portable sessions, dotclaude profiles, git safety hooks, multi-backend support
2. **Only blackdot with Feature Registry architecture** - Central control plane with presets and dependency resolution
3. **Only blackdot with Configuration Layers** - 5-layer priority system (env → project → machine → user → defaults)
4. **Only blackdot with multi-vault backend support** - Bitwarden, 1Password, or pass with unified API
5. **Only blackdot with comprehensive health checks** - Validator with auto-fix
6. **Only blackdot with drift detection** - Compare local vs vault state
7. **Only blackdot with schema validation** - Ensures SSH keys/configs are valid before restore
8. **Only blackdot with Docker bootstrap testing** - Reproducible CI/CD environments
9. **Only blackdot with machine-specific templates** - Auto-generate configs for work vs personal machines
